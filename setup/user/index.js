...
const start = (bocchi = new Client()) => {
    ...
    bocchi.onGlobalParticipantsChanged(async (event) => {
        ...
        try {
            const welcomeSettings = _welcome.find(entry => entry.groupId === event.chat)
            let welcomeMessage = `Welcome in *${gcChat.contact.name}*, *@${pushname}*! You are the *${groupMetadata.participants.length}.* member.`
            if (welcomeSettings && welcomeSettings.message) {
                welcomeMessage = welcomeSettings.message
            }
            if (event.action === 'add' && event.who !== botNumbers && welcomeSettings) {
                await bocchi.sendText(event.chat, welcomeMessage)
            } else if (event.action === 'remove' && event.who !== botNumbers && welcomeSettings) {
                await bocchi.sendText(event.chat, `~ *${pushname}* has left the group.`)
            }
        } catch (err) {
            console.error(err)
        }
